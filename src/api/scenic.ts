...
export const getScenicPage = (params: ScenicPageParams) => {
  return request.post<any, PageResult<ScenicSpot>>(
    '/scenic-spots/page',
    params,
  );
};
...
export const favoriteScenic = (id: number) => {
  return request.post<any, null>(`/favorites/SCENIC/${id}`);
};

// 取消收藏景点
export const unfavoriteScenic = (id: number) => {
  return request.delete<any, null>(`/favorites/SCENIC/${id}`);
};

// 检查是否收藏（返回boolean）
export const checkFavorite = (id: number) => {
  return request.get<any, boolean>(`/favorites/check/SCENIC/${id}`);
};
